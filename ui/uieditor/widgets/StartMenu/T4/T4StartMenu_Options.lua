...
DataSources.T4StartMenuOptionsTabs = ListHelper_SetupDataSource( "T4StartMenuOptionsTabs", function ( controller )
	local tabList = {}
    ...
    if Engine.IsInGame() then
        if Engine.IsZombiesGame() then
			table.insert( tabList, {
				models = { tabName = "MENU_OPTIONS_CONTROLS_CAPS", tabWidget = "CoD.StartMenu_Options", tabIcon = "" },
				properties = { tabId = "options" }
			} )
        end
    end
...
LUI.createMenu.T4StartMenu_Options = function ( controller )
	...
	self.FETabBar.Tabs.grid:setDataSource( "T4StartMenuOptionsTabs" )
