	...
	self:mergeStateConditions( {
		...
		},
		{
		    ...
		    condition = function ( menu, element, event )
		        ...
		        local GetValue = function( name )
		            local model = Engine.GetModel( controllerModel, "currentWeapon." .. name )
		            return model and Engine.GetModelValue( model ) or 0
		        end
		        ...
		        local ammoInClip = GetValue( "ammoInClip" )
		        local ammoInDWClip = GetValue( "ammoInDWClip" )
		        local ammoStock = GetValue( "ammoStock" )
		        local clipMaxAmmo = GetValue( "clipMaxAmmo" )
		        local clipMaxAmmoDW = GetValue( "clipMaxAmmoDW" )

		        local totalMagCapacity = clipMaxAmmo + ( clipMaxAmmoDW > 0 and clipMaxAmmoDW or 0 )
		        local totalCurrentClip = ammoInClip + ( ammoInDWClip > 0 and ammoInDWClip or 0 )

		        if ammoStock == 0 and totalCurrentClip == 0 then
		            return false
		        end

		        return ammoStock < totalMagCapacity
		    end
		},
		{
			stateName = "Reload",
