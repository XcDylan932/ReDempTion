require( "ui.uieditor.widgets.HUD.T4.T4AmmoWidget.T4AmmoEquipment" )
require( "ui.uieditor.widgets.HUD.T4.T4AmmoWidget.T4AmmoInfo" )
require( "ui.uieditor.widgets.HUD.T4.T4AmmoWidget.T4AmmoWarning" )
...
CoD.T4AmmoContainer.new = function( menu, controller )
	...
	self.T4GumMeter = CoD.ZmAmmo_BBGumMeterWidget.new( menu, controller )
    self.T4GumMeter:setLeftRight( false, true, -106 - 5, -56 - 5 )
    self.T4GumMeter:setTopBottom( true, false, 486 + 170, 539 + 170 )
    self.T4GumMeter:setScale( 0.85 )
    self:addElement( self.T4GumMeter )

    self.T4GumsRemaining = LUI.UIText.new()
	self.T4GumsRemaining:setLeftRight( false, true, -106 + 21.5 + 21.5, -96 + 21.5 + 21.5 )
	self.T4GumsRemaining:setTopBottom( true, false, 504.5 + 42.5 + 140, 520.5 + 42.5 + 140 )
	self.T4GumsRemaining:setTTF( "fonts/itc_legacy_sans_bold.ttf" )
	self.T4GumsRemaining:setScale( 1.2 )
	self.T4GumsRemaining:setAlignment( Enum.LUIAlignment.LUI_ALIGNMENT_CENTER )
	self.T4GumsRemaining:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "bgb_activations_remaining" ), function( model )
	    local modelValue = Engine.GetModelValue( model )
	    if modelValue then
	        if modelValue > 0 and modelValue < 6 then
	            self.T4GumsRemaining:setText( modelValue )
	        else
	            self.T4GumsRemaining:setText( "" )
	        end
	    end
	end )
	self:addElement( self.T4GumsRemaining )
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function()
				...
				self.T4GumMeter:completeAnimation()
				self.T4GumMeter:setAlpha( 0 )
				self.clipFinished( self.T4GumMeter, {} )

				self.T4GumsRemaining:completeAnimation()
				self.T4GumsRemaining:setAlpha( 0 )
				self.clipFinished( self.T4GumsRemaining, {} )
			end,
			...
			HudStart = function()
				...
				self.T4GumMeter:completeAnimation()
				self.T4GumMeter:setAlpha( 0 )
				HudStartTransition( self.T4GumMeter, {} )

				self.T4GumsRemaining:completeAnimation()
				self.T4GumsRemaining:setAlpha( 0 )
				HudStartTransition( self.T4GumsRemaining, {} )
			end
		},
		...
		HudStart = {
			DefaultClip = function()
				...
				self.T4GumMeter:completeAnimation()
				self.T4GumMeter:setAlpha( 1 )
				self.clipFinished( self.T4GumMeter, {} )

				self.T4GumsRemaining:completeAnimation()
				self.T4GumsRemaining:setAlpha( 1 )
				self.clipFinished( self.T4GumsRemaining, {} )
			end,
			...
			DefaultState = function()
				...
				self.T4GumMeter:completeAnimation()
				self.T4GumMeter:setAlpha( 1 )
				DefaultStateTransition( self.T4GumMeter, {} )

				self.T4GumsRemaining:completeAnimation()
				self.T4GumsRemaining:setAlpha( 1 )
				DefaultStateTransition( self.T4GumsRemaining, {} )
			end
		}
	}
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function( element )
		...
		element.T4GumMeter:close()
		--element.T4GumRemaining:close()
		element.T4GumsRemaining:close()
	end )
