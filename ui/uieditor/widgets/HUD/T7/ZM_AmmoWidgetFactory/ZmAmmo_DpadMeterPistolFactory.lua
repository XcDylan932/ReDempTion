local PostLoadFunc = function( self, controller, menu )
    self.UpdateColors = function( self )
        local elements = { self.SwordRingBack, self.AbilitySwirl, self.SwordRing, self.SwordRingEdge }
...
CoD.ZmAmmo_DpadMeterPistolFactory.new = function ( menu, controller )
	...
	self.SwordRingEdge = LUI.UIImage.new()
	self.SwordRingEdge:setLeftRight( true, false, -22, 64 )
	self.SwordRingEdge:setTopBottom( true, false, -14.5, 71.5 )
	self.SwordRingEdge:setImage( RegisterImage( "uie_t7_zm_derriese_hud_ammo_dpadmtr_edge" ) )
	self.SwordRingEdge:setMaterial( LUI.UIImage.GetCachedMaterial( "uie_clock_normal" ) )
	self.SwordRingEdge:setShaderVector( 1, 0.5, 0, 0, 0 )
	self.SwordRingEdge:setShaderVector( 2, 0.5, 0, 0, 0 )
	self.SwordRingEdge:setShaderVector( 3, 0.23, -0.23, 0, 0 )
	self.SwordRingEdge:subscribeToGlobalModel( controller, "PerController", "zmhud.swordEnergy", function ( model )
		local zmhudSwordEnergy = Engine.GetModelValue( model )
		if zmhudSwordEnergy then
			self.SwordRingEdge:setShaderVector( 0, AdjustStartEnd( 0.06, 0.94, CoD.GetVectorComponentFromString( zmhudSwordEnergy, 1 ), CoD.GetVectorComponentFromString( zmhudSwordEnergy, 2 ), CoD.GetVectorComponentFromString( zmhudSwordEnergy, 3 ), CoD.GetVectorComponentFromString( zmhudSwordEnergy, 4 ) ) )
		end
	end )
	self:addElement( self.SwordRingEdge )
	
	self.SwordRingRed = LUI.UIImage.new()
	self.SwordRingRed:setLeftRight( true, false, -23, 63 )
	self.SwordRingRed:setTopBottom( true, false, -14.5, 71.5 )
	self.SwordRingRed:setAlpha( 0 )
	self.SwordRingRed:setImage( RegisterImage( "uie_t7_zm_derriese_hud_ammo_dpadmtr_red" ) )
	self.SwordRingRed:setMaterial( LUI.UIImage.GetCachedMaterial( "uie_clock_normal" ) )
	self.SwordRingRed:setShaderVector( 1, 0.5, 0, 0, 0 )
	self.SwordRingRed:setShaderVector( 2, 0.5, 0, 0, 0 )
	self.SwordRingRed:setShaderVector( 3, 0.05, 0, 0, 0 )
	self.SwordRingRed:subscribeToGlobalModel( controller, "PerController", "zmhud.swordEnergy", function ( model )
		local zmhudSwordEnergy = Engine.GetModelValue( model )
		if zmhudSwordEnergy then
			self.SwordRingRed:setShaderVector( 0, AdjustStartEnd( 0.06, 0.94, CoD.GetVectorComponentFromString( zmhudSwordEnergy, 1 ), CoD.GetVectorComponentFromString( zmhudSwordEnergy, 2 ), CoD.GetVectorComponentFromString( zmhudSwordEnergy, 3 ), CoD.GetVectorComponentFromString( zmhudSwordEnergy, 4 ) ) )
		end
	end )
	self:addElement( self.SwordRingRed )
	...
	self.clipsPerState = {
		DefaultState = {
			DefaultClip = function ()
				...
				self.SwordRingRed:completeAnimation()
				self.SwordRingRed:setAlpha( 0 )
				self.clipFinished( self.SwordRingRed, {} )
			end
		},
		...
		Ready = {
			DefaultClip = function ()
				...
				local SwordRingEdgeFrame2 = function ( element, event )
					if not event.interrupted then
						element:beginAnimation( "keyframe", 1299, false, false, CoD.TweenType.Linear )
					end
					element:setAlpha( 0 )
					if event.interrupted then
						self.clipFinished( element, event )
				...
				self.SwordRingEdge:completeAnimation()
				self.SwordRingEdge:setAlpha( 1 )
				SwordRingEdgeFrame2( self.SwordRingEdge, {} )
		...
		Charge = {
			DefaultClip = function ()
				...
				self.SwordRingEdge:setAlpha( 1 )
				self.clipFinished( self.SwordRingEdge, {} )
