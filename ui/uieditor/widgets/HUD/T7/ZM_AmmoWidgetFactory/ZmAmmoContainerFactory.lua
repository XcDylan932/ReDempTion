...
require( "ui.uieditor.widgets.HUD.T7.ZM_AmmoWidgetFactory.ZmAmmoFactory" )

local PreLoadFunc = function( self, controller )
	local controllerModel = Engine.GetModelForController( controller )
	Engine.CreateModel( controllerModel, "colorSettings_UI" )
end

local PostLoadFunc = function( self, controller )
	self:subscribeToModel( Engine.GetModel( Engine.GetGlobalModel(), "fastRestart" ), function( model )
        CoD.PCUtil.ResetGlobalSettings()
    end )
	--CoD.PCUtil.ResetGlobalSettings()
end
...
CoD.ZmAmmoContainerFactory.new = function ( menu, controller )
	...
	self.ZmAmmo = CoD.ZmAmmoFactory.new( menu, controller )
	...
	self.ZmAmmo:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "currentWeapon.viewmodelWeaponName" ), function ( model )
		menu:updateElementState( self.ZmAmmo, { name = "model_validation", menu = menu, modelValue = Engine.GetModelValue( model ), modelName = "currentWeapon.viewmodelWeaponName" } )
	end )
