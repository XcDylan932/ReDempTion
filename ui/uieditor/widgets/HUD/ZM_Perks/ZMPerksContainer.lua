...
local activePerksTable = CoD.ZMPerksTable[2] 
		...
		if perkStatus ~= nil and perkStatus > 0 then
			...
			if not existingIndex then
				...
				if item.models.image ~= value or item.models.status ~= perkStatus then
					...
					local listModel = Engine.GetModel( Engine.GetModel( Engine.GetModelForController( controller ), "ZMPerks" ), tostring( existingIndex ) )
					if listModel then
...
DataSources.ZMPerks = DataSourceHelpers.ListSetup( "ZMPerks", function( controller, element )
...
CoD.ZMPerksContainer.new = function( menu, controller )
	...
	self.PerkList:setLeftRight( true, false, 0, 800 )
	self.PerkList:setTopBottom( false, true, -36, 0 )
	self.PerkList:setWidgetType( CoD.PerkListItemFactory )
	self.PerkList:setHorizontalCount( 15 )
	self.PerkList:setDataSource( "ZMPerks" )
