...
local SetAttachmentModels = function ( self, controller )
	...
	local currentWeaponName = Engine.GetModelValue( Engine.GetModel( currentWeaponModel, "weaponName" ) )
	if self.currentWeapon ~= currentWeaponName then
		self.AttachmentInfo:UpdateAttachments( self.currentAttachment1, self.currentAttachment2, self.currentAttachment3 )
		self.currentWeapon = currentWeaponName
...
local PostLoadFunc = function ( self, controller )
	self:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "currentWeapon.updateWeaponSelect" ), function ( model )
		SetAttachmentModels( self, controller )
	end )
end
...
CoD.ZmAmmo_AttachmentInfo.new = function ( menu, controller )
	...
	CoD.UIColors.SetElementColor( self.Label0, CoD.PCUtil.GameOptions[controller]["colorSettings_UI"] )
	...
	self.Label0:subscribeToGlobalModel( controller, "CurrentWeapon", "aat", function ( model )
		...
		if aat then
			self.Label0:setText( Engine.Localize( LocalizeString( aat ) ) )
		end
	end )
