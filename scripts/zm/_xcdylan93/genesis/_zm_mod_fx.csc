...
function init_fx()
{
    foreach( category in array( "powerup", "muzzle", "laser", "tesla", "tgun", "idgun" ) )
    {
...
function play_idgun_breath( localclientnum )
{
    ...
    for(;;)
    {
        ...
        for( i = 0; i < tubes.size; i++ )
        {
            ...
            {
                fx = self get_fx_path( "idgun_pers" );
                self.weapon_fx[i] = PlayViewmodelFX( localclientnum, fx, tubes[i] );
            }
            wait 0.15;
        }
